...
def create_mocked_llm_agent(
    jid: str = "llm_test@localhost", password: str = "test_password", provider: Optional[BaseLLMProvider] = None
):
...
def create_mock_tool_with_result(name: str, result: any, should_error: bool = False):
    ...
    return LLMTool(
        ...
        parameters={"type": "object", "properties": {"input": {"type": "string", "description": "Input parameter"}}},
        func=tool_func,
    )


def create_mock_conversation_state(conversation_id: str, interaction_count: int = 1, state: str = "active"):
    """
...
def create_routing_function(target_jid: Optional[str] = None, transform_func: Optional[Callable] = None):
    """
...
def create_tool_call_response(tool_name: str, tool_args: dict, call_id: str = "call_123"):
    """
    ...
    """
    return {"id": call_id, "name": tool_name, "arguments": tool_args}
