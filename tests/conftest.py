...
def context_manager():
    ...
    return ContextManager(max_tokens=4096, system_prompt="You are a helpful test assistant.")
...
def mock_simple_tool():
    ...
    return LLMTool(
        ...
        parameters={
            "type": "object",
            "properties": {"text": {"type": "string", "description": "Text to process"}},
            "required": ["text"],
        },
        func=simple_func,
    )
...
def mock_async_tool():
    ...
    return LLMTool(
        ...
        parameters={
            "type": "object",
            "properties": {"number": {"type": "integer", "description": "Number to process"}},
            "required": ["number"],
        },
        func=async_func,
    )
...
class MockLLMProvider(BaseLLMProvider):
    ...
    async def get_llm_response(
        ...
        if output_schema is not None and not tools and self.structured_responses:
            ...
            result = {"text": None, "tool_calls": [], "structured": self.structured_responses[idx]}
            self.call_count += 1
            return result

        # Return tool calls if specified, otherwise return text response
        if self.tool_calls and self.call_count < len(self.tool_calls):
            result = {"text": None, "tool_calls": self.tool_calls[self.call_count], "structured": None}
        else:
            response_index = self.call_count % len(self.responses)
            result = {"text": self.responses[response_index], "tool_calls": [], "structured": None}
...
def mock_llm_provider_with_tools():
    ...
    return MockLLMProvider(responses=["Final response after tool use"], tool_calls=tool_calls)
...
def subagent_ids():
    ...
    return ["subagent1@localhost", "subagent2@localhost", "subagent3@localhost"]
...
def mock_coordinator_provider():
    ...
    return MockLLMProvider(responses=["Coordination complete. <TASK_COMPLETE>"], tool_calls=tool_calls)
...
def sample_document():
    ...
    return Document(content="Sample document content for testing", metadata={"source": "test", "type": "sample"})
...
def sample_documents():
    ...
    return [Document(content=f"Document {i} content", metadata={"index": i, "source": "test"}) for i in range(5)]
