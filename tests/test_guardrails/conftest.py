...
def basic_keyword_guardrail():
    ...
    return KeywordGuardrail(
        name="test_keyword", blocked_keywords=["bad", "harmful", "inappropriate"], action=GuardrailAction.BLOCK
    )
...
def email_regex_guardrail():
    ...
    return RegexGuardrail(
        name="email_redactor", patterns={r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b": "[EMAIL]"}
    )
...
def simple_custom_function():
    ...
    def check_length(content: str, context: Dict[str, Any]) -> GuardrailResult:
        if len(content) > 100:
            return GuardrailResult(action=GuardrailAction.BLOCK, reason="Content too long")
    ...
    def add_prefix(content: str, context: Dict[str, Any]) -> GuardrailResult:
        if not content.startswith("[CHECKED]"):
            return GuardrailResult(
                action=GuardrailAction.MODIFY, content=f"[CHECKED] {content}", reason="Added safety prefix"
            )
        return GuardrailResult(action=GuardrailAction.PASS, content=content)
...
async def async_custom_function():
    ...
    async def async_check(content: str, context: Dict[str, Any]) -> GuardrailResult:
        ...
        if "async_block" in content:
            return GuardrailResult(action=GuardrailAction.BLOCK, reason="Async block triggered")
        return GuardrailResult(action=GuardrailAction.PASS, content=content)
...
def custom_function_guardrail(simple_custom_function):
    ...
    return CustomFunctionGuardrail(name="length_checker", check_function=simple_custom_function)


@pytest.fixture
def test_context():
    """Create a basic test context."""
    return {"conversation_id": "test_conv_123", "sender": "user@example.com", "message": Mock(spec=Message)}


@pytest.fixture
def guardrail_result_pass():
    """Create a PASS guardrail result."""
    return GuardrailResult(action=GuardrailAction.PASS, content="safe content", reason="Content is safe")
...
def guardrail_result_block():
    ...
    return GuardrailResult(
        action=GuardrailAction.BLOCK, reason="Content blocked for safety", custom_message="Your content was blocked"
    )
...
def trigger_callback_log():
    ...
    def callback(result: GuardrailResult):
        log.append({"action": result.action, "reason": result.reason, "content": result.content})
