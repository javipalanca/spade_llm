...
if not LLM_MODEL or not EMBEDDING_MODEL:
    raise SystemExit("LLM_MODEL and EMBEDDING_MODEL must be set — copy .env.example to .env and configure them.")
...
async def setup_agents(
    retriever: VectorStoreRetriever, llm_provider: LLMProvider
) -> Tuple[RetrievalAgent, LLMAgent, LLMAgent]:
...
async def run_comparison_demo(llm_with_rag: LLMAgent, llm_no_rag: LLMAgent, retriever: VectorStoreRetriever):
    ...
    if retrieved_docs:
        ...
        for idx, doc in enumerate(retrieved_docs, 1):
            ...
            docs_table.add_row(f"#{idx}", source_display, preview)
