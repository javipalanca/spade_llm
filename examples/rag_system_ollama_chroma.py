...
async def demonstrate_filtered_search(retriever: VectorStoreRetriever):
    ...
    results = await retriever.retrieve(
        "programming and development tools", k=3, search_type="similarity", filters={"category": "tools"}
    )
...
async def demonstrate_document_management(vector_store: Chroma):
    ...
    if ids_to_delete:
        ...
        console.print(f"   [{status_style}]{status}[/{status_style}]: {len(ids_to_delete)} chunk(s) from 'ollama_info'")
    else:
